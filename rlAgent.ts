...
import { LEARNING_RATE, DISCOUNT_FACTOR, EPSILON_DECAY, MIN_EPSILON, INITIAL_EPSILON } from "./config";

type State = string; // We'll encode state as a string
...
export class RLAgent {
  ...
  constructor(map: MapGrid, worm: Worm, token: Token) {
    this.map = map;
    this.worm = worm;
    this.token = token;
  }
  ...
  chooseAction(state: State): Direction {
    ...
    return chosen as Direction;
  }

  updateQ(prevState: State, action: Direction, reward: number, nextState: State) {
    ...
    // Q-learning update
    qValues[action] = oldQ + LEARNING_RATE * (reward + DISCOUNT_FACTOR * maxNextQ - oldQ);
